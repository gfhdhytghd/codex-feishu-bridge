...
const STREAM_DEFAULTS: Record<string, StreamConfig> = {
  ...
  feishu: { intervalMs: 1200, minDeltaChars: 30, maxChars: 26000 },
};
...
async function handleMessage(
  ...
  const onPartialText = (previewState && streamCfg) ? (fullText: string, options?: { force?: boolean }) => {
  ...
  try {
    ...
    if (activeRecord.guided) {
      ...
      if (
        ...
        const finalPreviewText = result.previewText || result.responseText;
        const finalText = finalPreviewText.length > streamCfg!.maxChars
          ? finalPreviewText.slice(0, streamCfg!.maxChars) + '...'
          : finalPreviewText;
        const previewResult = await adapter.sendPreview(msg.address.chatId, finalText, previewState.draftId);
        deliveredByPreview = previewResult === 'sent';
        if (previewResult === 'degrade') previewState.degraded = true;
      }
      if (!deliveredByPreview) {
