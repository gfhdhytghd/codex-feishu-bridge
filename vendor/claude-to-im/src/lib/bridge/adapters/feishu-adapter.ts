...
export class FeishuAdapter extends BaseChannelAdapter {
  ...
  async stop(): Promise<void> {
    ...
    this.previewCards.clear();
    this.previewUpdateChains.clear();
  ...
  async sendPreview(chatId: string, text: string, draftId: number): Promise<'sent' | 'skip' | 'degrade'> {
    const previous = this.previewUpdateChains.get(draftId) ?? Promise.resolve<'sent' | 'skip' | 'degrade'>('sent');
    const next = previous
      .catch((): 'skip' => 'skip')
      .then(() => this.sendPreviewNow(chatId, text, draftId));

    this.previewUpdateChains.set(draftId, next);

    try {
      return await next;
    } finally {
      if (this.previewUpdateChains.get(draftId) === next) {
        this.previewUpdateChains.delete(draftId);
      }
    }
  }

  private async sendPreviewNow(chatId: string, text: string, draftId: number): Promise<'sent' | 'skip' | 'degrade'> {
    ...
    if (!existing) {
      try {
        ...
        if (res?.data?.message_id) {
          this.previewCards.set(draftId, { chatId, messageId: res.data.message_id, lastText: text });
          return 'sent';
        }
        console.warn('[feishu-adapter] Streaming card create failed:', res?.msg, res?.code);
        return 'skip';
      } catch (err) {
        console.warn('[feishu-adapter] Streaming card create error:', err instanceof Error ? err.message : err);
        return 'skip';
      }
    }
    ...
    try {
      ...
      console.warn('[feishu-adapter] Streaming card patch failed:', res?.msg, res?.code);
      return 'skip';
    } catch (err) {
      const code = (err as { code?: number })?.code;
      console.warn('[feishu-adapter] Streaming card patch error:', err instanceof Error ? err.message : err);
      return code === 99991672 ? 'degrade' : 'skip';
    }
  }
  ...
  endPreview(chatId: string, draftId: number): void {
    ...
    void chain.finally(() => {
      const existing = this.previewCards.get(draftId);
      if (!existing || existing.chatId !== chatId || !this.restClient) return;
      this.previewCards.delete(draftId);
      this.previewUpdateChains.delete(draftId);
