...
build_env_dict() {
  ...
  case "$runtime" in
    codex|auto)
      ...
      # Forward custom provider secrets declared in ~/.codex/config.toml via env_key.
      # This lets launchd-managed bridge sessions reuse the user's existing Codex CLI
      # provider configuration, including third-party API services.
      ...
      if [ -f "$codex_config" ]; then
        while IFS= read -r var; do
          ...
          [ -z "$val" ] && continue
          dict+="${indent}<key>${var}</key>\n${indent}<string>${val}</string>\n"
        done < <(sed -nE 's/^[[:space:]]*env_key[[:space:]]*=[[:space:]]*"([^"]+)".*/\1/p' "$codex_config" | sort -u)
      fi
      ;;
  esac
  case "$runtime" in
...
start_detached_process() {
  ...
  nohup "$node_path" "$SKILL_DIR/dist/daemon.mjs" >>"$LOG_FILE" 2>&1 </dev/null &
}
...
supervisor_start() {
  ...
  echo "launchd bootstrap failed; falling back to detached background process" >&2
  start_detached_process
}
